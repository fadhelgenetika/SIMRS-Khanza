...
public class DlgBilingRanap extends javax.swing.JDialog {
    ...
    private double subttlBHP=0,subttlNON=0;
    ...
    private void prosesCariReg() {        
        ...
        try{  
            psreg=koneksi.prepareStatement(
                    "select reg_periksa.no_rkm_medis,concat(DATE_FORMAT(kamar_inap.tgl_masuk, '%e %M %Y'),' ',kamar_inap.jam_masuk) as registrasi,kamar_inap.kd_kamar,concat(if(kamar_inap.tgl_keluar='0000-00-00',DATE_FORMAT(CURDATE(), '%e %M %Y'),DATE_FORMAT(kamar_inap.tgl_keluar, '%e %M %Y')),' ',kamar_inap.jam_keluar) as keluar,  "+
    ...
    private void prosesCariObat(String norawat) {    
        ...
        try{
            pscariobat=koneksi.prepareStatement(
                    ...
                    "(sum(detail_pemberian_obat.total)-sum(detail_pemberian_obat.embalase+detail_pemberian_obat.tuslah)) as total, databarang.kdjns "+
            ...
            try {
                ...
                if(centangobatranap.equals("Yes")){
                    while(rscariobat.next()){
                        ...
                        if(rscariobat.getString("kdjns").equals("J038") || rscariobat.getString("kdjns").equals("J041") || rscariobat.getString("kdjns").equals("J039") 
                                || rscariobat.getString("kdjns").equals("J040") || rscariobat.getString("kdjns").equals("JA01") || rscariobat.getString("kdjns").equals("JA02")){
                            //BHP
                            subttlBHP=subttlBHP+rscariobat.getDouble("total")+rscariobat.getDouble("tambahan");
                        } else {
                            //NON BHP
                            subttlNON=subttlNON+rscariobat.getDouble("total")+rscariobat.getDouble("tambahan");
                        }
                    }
                }else{
                    while(rscariobat.next()){
        ...
        if(subttl>1){
            ttlobat=subttl;
            tabModeRwJlDr.addRow(new Object[]{true,"","Total BHP : "+Valid.SetAngka(subttlBHP),"",null,null,null,null,"TtlObatB"});
            tabModeRwJlDr.addRow(new Object[]{true,"","Total Obat : "+Valid.SetAngka(subttlNON),"",null,null,null,null,"TtlObatN"});
