...
//                                System.out.println("Menjalankan Query : select RESDT.ORDER_TESTID,RESDT.DATA_TYP,RESDT.RESULT_VALUE,RESDT.RESULT_FT,RESDT.UNIT,RESDT.FLAG,RESDT.REF_RANGE from RESDT where RESDT.ONO='"+order+"' and RESDT.TEST_NM='"+rstampil.getString("Pemeriksaan")+"'");
    ...
    private void tampilYAHMATEVY(String order) { 
        try {
            ...
            for(i2=0;i2<tbTarif.getRowCount();i2++){ 
                if(tbTarif.getValueAt(i2,0).toString().equals("true")){
                    ...
                    try {
                        ...
                        rstampil=pstampil.executeQuery();
                        while(rstampil.next()){
                            pstindakan=koneksiyahmatevy.prepareStatement("select noorder, kd_jenis_prw, id_template, nilai, nilai_rujukan, keterangan from detail_hasil_lab where noorder=? and id_template=?");
                            try {
                                pstindakan.setString(1,order);
                                pstindakan.setString(2,rstampil.getString("id_template"));
                                rstindakan=pstindakan.executeQuery();
                        ...
                        while(rstampil.next()){
                            ...
                            try {
                                ...
                                if(rslica.next()){
                                    ...
                                    if(!rslica.getString("temp5").trim().equals("")){
                                        //dari lis
                                        satuan=rslica.getString("temp5");
                                    } else {
                                        //dari simrs
                                        satuan=rstampil.getString("satuan");
                                    }
                                    
                                    if(!rslica.getString("temp4").trim().equals("")){
                                        //dari lis
                                        nn=rslica.getString("temp4");
                                    } else {
                                        //dari simrs
                                        nn=rstampil.getString("nilai_rujukan_ld");
                                    }
                                    keterangan="";
                                }
                            } catch (Exception e) {
    ...
    public void setOrderYAHMATEVY(String order,String norawat,String posisi){
        noorder=order;
        TNoRw.setText(norawat);
        this.status=posisi;
        isRawat();
                        ...
                        try {
                            ...
                            if(rscari.next()){  
                                ...
                                if(Sequel.menyimpantf2("detail_periksa_lab","?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?","ID Template",16,new String[]{
                                        ...
                                        tbPemeriksaan.getValueAt(i,6).toString(),tbPemeriksaan.getValueAt(i,2).toString(),tbPemeriksaan.getValueAt(i,4).toString(),tbPemeriksaan.getValueAt(i,3).toString(),
