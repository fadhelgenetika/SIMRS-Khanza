...
public class DlgCariPeriksaLab extends javax.swing.JDialog {
    ...
    public DlgCariPeriksaLab(java.awt.Frame parent, boolean modal) {
        ...
        jPopupMenu1.remove(MnCetakLab);
        jPopupMenu1.remove(MnPDFLab);
    ...
    private void initComponents() {
        ...
        MenuCetakNew = new javax.swing.JMenuItem();
        ...
        MenuCetakNew.setBackground(new java.awt.Color(255, 255, 254));
        MenuCetakNew.setFont(new java.awt.Font("Tahoma", 0, 11)); // NOI18N
        MenuCetakNew.setForeground(new java.awt.Color(50, 50, 50));
        MenuCetakNew.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/category.png"))); // NOI18N
        MenuCetakNew.setText("Cetak Hasil Lab");
        MenuCetakNew.setName("MenuCetakNew"); // NOI18N
        MenuCetakNew.setPreferredSize(new java.awt.Dimension(100, 26));
        MenuCetakNew.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                MenuCetakNewActionPerformed(evt);
            }
        });
        jPopupMenu1.add(MenuCetakNew);
...
private void tbDokterKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_tbDokterKeyPressed
   if(tabMode.getRowCount()!=0){
    ...
    private void MenuCetakNewActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MenuCetakNewActionPerformed
        this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
        if(tabMode.getRowCount()==0){
            JOptionPane.showMessageDialog(null,"Maaf, data sudah habis...!!!!");
            TCari.requestFocus();
...
//                        param.put("umur",rs.getString("umur"));
                        ...
                        param.put("tgl_lahir",Sequel.cariIsi("select DATE_FORMAT(pasien.tgl_lahir,'%d-%m-%Y') from reg_periksa,pasien where reg_periksa.no_rkm_medis=pasien.no_rkm_medis AND reg_periksa.no_rawat='"+rs.getString("no_rawat")+"'"));
                        param.put("no_rkm_medis",Sequel.cariIsi("select no_rkm_medis from reg_periksa where no_rawat='"+rs.getString("no_rawat")+"'"));
                        param.put("no_sep",Sequel.cariIsi("SELECT no_sep FROM bridging_sep WHERE no_rawat='"+rs.getString("no_rawat")+"' ORDER BY tglsep DESC"));
                        param.put("statuspasien",Sequel.cariIsi("select penjab.png_jawab from reg_periksa,penjab where reg_periksa.kd_pj=penjab.kd_pj AND reg_periksa.no_rawat='"+rs.getString("no_rawat")+"'"));
                        param.put("umur",Sequel.cariIsi("select concat(reg_periksa.umurdaftar,' ',reg_periksa.sttsumur) from reg_periksa where no_rawat='"+rs.getString("no_rawat")+"'"));
                        
                        pspermintaan=koneksi.prepareStatement(
                                "select noorder,DATE_FORMAT(tgl_permintaan,'%d-%m-%Y') as tgl_permintaan,jam_permintaan,DATE_FORMAT(tgl_sampel,'%d-%m-%Y') as tgl_sampel,jam_sampel,DATE_FORMAT(tgl_hasil,'%d-%m-%Y') as tgl_hasil,jam_hasil from permintaan_lab where "+
                                "no_rawat=? and tgl_hasil=? and jam_hasil=?");
                        try {
                            pspermintaan.setString(1,rs.getString("no_rawat"));
                            pspermintaan.setString(2,Valid.SetTgl(rs.getString("tgl_periksa")));
                            ...
                            if(rspermintaan.next()){
                                param.put("nopermintaan",rspermintaan.getString("noorder"));   
                                param.put("tanggalpermintaan",rspermintaan.getString("tgl_permintaan")+" "+rspermintaan.getString("jam_permintaan"));  
                                param.put("tanggalsampel",rspermintaan.getString("tgl_sampel")+" "+rspermintaan.getString("jam_sampel"));
                                param.put("tanggalhasil",rspermintaan.getString("tgl_hasil")+" "+rspermintaan.getString("jam_hasil"));
                                Valid.MyReport("rptPeriksaLabAktif.jasper","report","::[ Pemeriksaan Laboratorium ]::",param);   
                            }else{
                                Valid.MyReport("rptPeriksaLabAktif.jasper","report","::[ Pemeriksaan Laboratorium ]::",param);   
                            }
                        } catch (Exception e) {
        ...
        this.setCursor(Cursor.getDefaultCursor());
    }//GEN-LAST:event_MenuCetakNewActionPerformed
    ...
    */
    ...
    private javax.swing.JMenuItem MenuCetakNew;
